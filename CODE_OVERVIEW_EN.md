...
- **`build_mac_pkg.sh`** / **`build_win_pkg.sh`** / **`build_all.sh`** — build the installer packages. `VERSION=` near the top of the first two; `build_all.sh` runs both in sequence.
...
- **Version and changelog**: `APP_VERSION`, `APP_STAGE`, `CHANGELOG` — near the start of the file (around lines 60-115).
