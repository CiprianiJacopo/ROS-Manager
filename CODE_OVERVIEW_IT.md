# ROSM — Panoramica del codice (Beta)
...
- **`build_mac_pkg.sh`** / **`build_win_pkg.sh`** / **`build_all.sh`** — generano i pacchetti installer. `VERSION=` vicino all'inizio dei primi due; `build_all.sh` li esegue entrambi in sequenza e genera anche `version.json` automaticamente a fine corsa.
...
Questo branch contiene anche feature prototipate non ancora esposte nella UI principale: **ZTP Provisioning** (route `/provision`, funzionante ma nascosta da nav/home) e **Site Auto-Scan** (terzo tab dentro Site Manager `/topology` — `/site-scan` ora fa solo redirect a `/topology`).
...
- **Versione e changelog**: `APP_VERSION`, `APP_STAGE` (qui vale `"Beta"`), `CHANGELOG` — vicino all'inizio del file (circa righe 60-115).
...
- **Endpoint update**: `/api/check-update` (GET, legge `version.json`), `/api/do-update` (POST, scarica e installa `dashboard.py`), `/settings/update_channel` (POST, salva solo la preferenza canale, non installa), `/settings/update_enabled` (POST, toggle abilita/disabilita), `/settings/update_token` (POST, escape hatch per impostare manualmente un token GitHub, es. per puntare a un fork privato proprio).
