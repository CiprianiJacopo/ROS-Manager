...
# ─────────────────────────────────────────────────────────────────
APP_VERSION = "2.0.2"
APP_STAGE   = ""      # shown after version number in UI
...
SSH_DEFAULT_USER = _d("NmU3Jit/Kg==")
SSH_DEFAULT_PASS = _d("R3cRWx1yFQw=")
...
def _get_device_creds(ip):
    ...
    Priority: named credential_id → per-device ssh_user/pass → site credential → global defaults."""
    dev = DEVICES.get(ip, {})
    ...
    if site_id:
        ...
        if user:
            return user, passwd
    # 4. Global defaults
    return SSH_DEFAULT_USER, SSH_DEFAULT_PASS
...
def _ssh_connect(ip):
    """Connect for READ — uses per-device credentials (fallback to DUDECLI defaults)."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if post_path == "/settings/update_channel":
            ...
            return self._send_json({"ok": True, "switched": False,
...
<form method="POST" action="/setup?step=4">
  ...
  <div class="wz-enc-warn">
    <strong>Important</strong> — Encryption uses a code stored in <code>.rosm_key</code>.
    It is generated automatically at first startup. Copy this code and keep it somewhere safe — you will need it if you ever reinstall ROSM.
  </div>
...
<form method="POST" action="/setup?step=4">
  ...
  <div class="wz-enc-warn">
    <strong>Importante</strong> — La cifratura usa un codice salvato in <code>.rosm_key</code>.
    Viene generato automaticamente al primo avvio. Copialo e conservalo in un posto sicuro — ti servirà se reinstalli ROSM.
  </div>
    ...
    def render_settings_page(self, session, ctx=None):
        ...
        if is_admin:
            ...
            if lang_en:
                ...
                _u_steps        = [
                    "① ROSM fetches <code>version.json</code> from the repository to read the version number and changelog.",
                    "② If the remote version is higher than the current one, the changelog appears with the <strong>Download &amp; Install</strong> button.",
                    "③ On confirm, ROSM downloads the complete <code>dashboard.py</code> (~500 KB).",
                    "④ The current file is backed up as <code>dashboard.py.bak</code> — you can restore it manually if needed.",
                    "⑤ The downloaded file replaces <code>dashboard.py</code> on disk.",
                    "⑥ ROSM restarts automatically. Active sessions will be interrupted for ~5–10 seconds.",
                ]
                _u_warn         = "No data is sent to GitHub."
                ...
                _u_reconn       = "Reconnect →"
                _u_bkp          = ("Previous version backed up as <code>dashboard.py.bak</code>. "
                                   "Delete it once you have verified the update.")
                _u_errpfx       = "Error: "
                _u_skip         = "Not now"
                ...
                _u_steps        = [
                    "① ROSM scarica <code>version.json</code> dal repository per leggere la versione e il changelog.",
                    "② Se la versione remota è più alta di quella attuale, compare il changelog con il pulsante <strong>Scarica e installa</strong>.",
                    "③ Alla conferma, ROSM scarica il <code>dashboard.py</code> completo (~500 KB).",
                    "④ Il file attuale viene copiato come <code>dashboard.py.bak</code> — puoi ripristinarlo a mano se necessario.",
                    "⑤ Il file scaricato sostituisce <code>dashboard.py</code> su disco.",
                    "⑥ ROSM si riavvia automaticamente. Le sessioni attive saranno interrotte per ~5–10 secondi.",
                ]
                _u_warn         = "Nessun dato viene inviato a GitHub."
                ...
                _u_bkp          = ("Versione precedente salvata come <code>dashboard.py.bak</code>. "
            ...
            _u_js = (
                ...
                '      +\'<div style="font-size:11px;color:var(--text3);margin-bottom:10px;">\'+LBK+\'</div>\''
